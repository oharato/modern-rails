...
function getResource(res, parent, props, custom, urn) {
    ...
    debuggable_1.debuggablePromise(resopAsync
        ...
        .catch((err) => {
        state_1.failPendingRegistration(res, err);
        done();
        throw err;
    }), label);
}
...
function registerResource(res, parent, t, name, custom, remote, newDependency, props, opts, sourcePosition, stackTrace, packageRef) {
    ...
    debuggable_1.debuggablePromise(resopAsync
        ...
        runAsyncResourceOp(opLabel, async () => {
            ...
            const unknown = custom && !resultFailed && !settings_1.isDryRun() && resp.getUnknown();
            // Now resolve the output properties.
            await resolveOutputs(res, t, name, props, resp.getObject(), deps, resop.resolvers, effectiveErr, resultFailed || unknown);
            done();
        });
    })
        .catch((err) => {
...
async function prepareResource(label, res, parent, custom, remote, props, opts, type, name) {
    ...
    const pendingRegistrations = state_1.getPendingResourceRegistrations();
    const pending = { res, parent, label, type, name, phase: "dependencies" };
    pending.fail = (err) => {
        resolveURN("", err);
