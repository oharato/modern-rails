 ...
 * @internal
 */
function getMonitorSupportedFeatures(monitorClient) {
    return new Promise((resolve, reject) => {
        monitorClient.getDeploymentInfo(new emptyproto.Empty(), (err, resp) => {
            if (err && err.code === grpc.status.UNIMPLEMENTED) {
...
async function awaitFeatureSupport() {
    ...
    if (features === undefined) {
        const probed = await Promise.all(Object.keys(legacyFeatureMapping)
            .sort()
            .map(async (id) => (await monitorSupportsFeature(monitorRef, id)) ? legacyFeatureMapping[id] : undefined));
        features = probed.filter((f) => f !== undefined);
    }
    const store = state_1.getStore();
    store.supportsSecrets = features.includes(resproto.ResourceMonitorFeature.RESOURCE_MONITOR_FEATURE_SECRETS);
    store.supportsResourceReferences = features.includes(resproto.ResourceMonitorFeature.RESOURCE_MONITOR_FEATURE_RESOURCE_REFERENCES);
    store.supportsOutputValues = features.includes(resproto.ResourceMonitorFeature.RESOURCE_MONITOR_FEATURE_OUTPUT_VALUES);
    store.supportsDeletedWith = features.includes(resproto.ResourceMonitorFeature.RESOURCE_MONITOR_FEATURE_DELETED_WITH);
    store.supportsReplaceWith = features.includes(resproto.ResourceMonitorFeature.RESOURCE_MONITOR_FEATURE_REPLACE_WITH);
    ...
    store.supportsInvokeTransforms = features.includes(resproto.ResourceMonitorFeature.RESOURCE_MONITOR_FEATURE_INVOKE_TRANSFORMS);
    store.supportsParameterization = features.includes(resproto.ResourceMonitorFeature.RESOURCE_MONITOR_FEATURE_PARAMETERIZATION);
    store.supportsResourceHooks = features.includes(resproto.ResourceMonitorFeature.RESOURCE_MONITOR_FEATURE_RESOURCE_HOOKS);
    store.supportsErrorHooks = features.includes(resproto.ResourceMonitorFeature.RESOURCE_MONITOR_FEATURE_ERROR_HOOKS);
    store.supportsInvokeDependsOn = features.includes(resproto.ResourceMonitorFeature.RESOURCE_MONITOR_FEATURE_INVOKE_DEPENDS_ON);
}
